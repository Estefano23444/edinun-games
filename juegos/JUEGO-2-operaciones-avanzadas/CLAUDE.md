...
**Juego: Operaciones avanzadas.** Carpeta autocontenida del repo
multi-juego `edinun-games`. **Cuatro niveles** (única excepción del repo
con un cuarto chip): sumas y restas verticales de 3 cifras, las mismas en
formato horizontal, las cuatro operaciones con decimales (resultado hasta
3 decimales), y suma/resta de enteros con signo (con resultados negativos).
Detalle del diseño en `.planning/operaciones-avanzadas-design.md`. La
mecánica visual del cartel central cambia por nivel (`problem.mode`):
`vert` reusa la rejilla CDU, `horiz` la mueve a una sola línea, `decimals`
agrega coma fija entre slots, `ints` agrega slot de signo y bandeja con
fichas `+`/`−`.

EDINUN GAMES en general — juegos de matemáticas para estudiantes (audiencia
6–11 años; el botón de nivel deja al estudiante elegir según lo que pueda
resolver). En móvil el diseño es horizontal pero el dispositivo se
sostiene **vertical**: el usuario gira físicamente el teléfono para verlo
bien (el contenido NO rota junto con la orientación del sistema).
