...
function EquationCanvas({ problem, answer, slots, slotsSpec, firstDigitIdx, feedback, reveal, setAnswer, eraseAt }) {
  ...
  if (slotsSpec.signSlot && (answer[0] === undefined || answer[0] === "")) {
    activeIdx = 0;
  } else if (problem.mode === "ints") {
    // ints: el número se ingresa de izquierda a derecha (centena → decena → unidad),
    // como se escribe un número, después del signo.
    for (let k = firstDigitIdx; k < slots; k++) {
  ...
  const wrapperBase = problem.mode === "vert"
    ? { position: "absolute", top: 158, bottom: 92, left: "50%", transform: "translateX(-50%)", textAlign: "center", display: "flex", flexDirection: "column", justifyContent: "center" }
...
function GameScreen({ app, setApp, go }) {
  ...
  function press(d) {
    ...
    setAnswer((prev) => {
      ...
      // Modo ints: el número se ingresa de IZQUIERDA A DERECHA (centena →
      // decena → unidad), como se escribe; los demás modos llenan de derecha
      // a izquierda (unidades primero, estilo cálculo manual).
      if (problem.mode === "ints") {
        for (let i = firstDigitIdx; i < slots; i++) {
  ...
  function erase() {
    ...
    setAnswer((prev) => {
      ...
      while (next.length < slots) next.push(undefined);
      if (problem.mode === "ints") {
        // ints: el número se escribe izq→der tras el signo, así que el último
        // colocado es el dígito más a la DERECHA lleno. Solo cuando ya no quedan
        // dígitos se borra el signo.
        for (let i = slots - 1; i >= firstDigitIdx; i--) {
          if (next[i] !== undefined) { next[i] = undefined; return next; }
        }
        if (next[0] !== undefined) { next[0] = undefined; return next; }
        return next;
      }
      // Otros modos (dígitos de derecha a izquierda): el último colocado es el
      // dígito más a la izquierda lleno.
