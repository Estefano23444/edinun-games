    ...
    return (
      ...
      >
        <div className="ed-glyphs" style={{ color: "rgba(255,255,255,0.10)", ...glyphsStyle }}>
          <span style={{ left: "6%", top: "12%", "--rot": "-8deg" }}>x</span>
          <span style={{ left: "82%", top: "16%", "--rot": "6deg", fontSize: "0.62em" }}>y</span>
          <span style={{ left: "10%", top: "78%", "--rot": "12deg", fontSize: "0.7em" }}>→</span>
          <span style={{ left: "88%", top: "72%", "--rot": "-10deg", fontSize: "0.55em" }}>↑</span>
          <span style={{ left: "45%", top: "8%", "--rot": "4deg", fontSize: "0.5em" }}>0</span>
          <span style={{ left: "3%", top: "45%", "--rot": "-4deg", fontSize: "0.6em" }}>(x,y)</span>
          <span style={{ left: "92%", top: "45%", "--rot": "8deg", fontSize: "0.58em" }}>3</span>
          <span style={{ left: "30%", top: "55%", "--rot": "-6deg", fontSize: "0.5em" }}>2</span>
  ...
  return (
    <>
      ...
      <div className="ed-glyphs" style={glyphsStyle}>
        <span style={{ left: "5%", top: "10%", "--rot": "-8deg" }}>(x,y)</span>
        <span style={{ left: "84%", top: "6%", "--rot": "6deg", fontSize: "0.78em" }}>×</span>
        <span style={{ left: "92%", top: "72%", "--rot": "-12deg", fontSize: "0.74em" }}>A</span>
        <span style={{ left: "3%", top: "82%", "--rot": "12deg", fontSize: "0.91em" }}>B</span>
        <span style={{ left: "46%", top: "4%", "--rot": "-4deg", fontSize: "0.59em" }}>x</span>
        <span style={{ left: "7%", top: "46%", "--rot": "-4deg", fontSize: "0.67em" }}>y</span>
        <span style={{ left: "88%", top: "40%", "--rot": "8deg", fontSize: "0.65em" }}>=</span>
        <span style={{ left: "22%", top: "22%", "--rot": "10deg", fontSize: "0.52em" }}>1</span>
        <span style={{ left: "70%", top: "24%", "--rot": "-6deg", fontSize: "0.57em" }}>2</span>
        <span style={{ left: "32%", top: "70%", "--rot": "8deg", fontSize: "0.61em" }}>3</span>
        <span style={{ left: "62%", top: "78%", "--rot": "-10deg", fontSize: "0.63em" }}>4</span>
        <span style={{ left: "18%", top: "58%", "--rot": "14deg", fontSize: "0.48em" }}>0</span>
...
// sin backend.
//
// IMPORTANTE: Antes este hook incrementaba el contador al montar HomeScreen.
// Eso era incorrecto: cada recarga de página o nueva pestaña sumaba una
// "visita" aunque el usuario no llegara a jugar. Ahora solo lee el contador
// guardado y expone markFirstAttempt(), que GameScreen llama la primera vez
// que el estudiante completa un intento (verify() con todos los slots
// llenos). Persistencia anti-doble-conteo en sessionStorage.
function useVisitorCount() {
  ...
  // Listener para que el contador se actualice si markFirstAttempt() corre
  // desde otra pantalla (GameScreen) en la misma sesión.
  useEffect(() => {
    function onUpdate() {
      const raw = localStorage.getItem(KEY);
      const v = raw ? parseInt(raw, 10) : 0;
...
// Marca la sesión actual como una visita real. Se llama desde GameScreen
// la primera vez que el jugador completa un intento (no al montar Home).
// Idempotente por sessionStorage: si ya se contó esta pestaña, no vuelve
// a contar aunque se llame varias veces.
    ...
    <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 22, maxWidth: 520 }}>
          ...
          {/* Selección de nivel — 2 chips (basic / medium). Naranja y
              amarillo, mismos colores que en operaciones-basicas para
              coherencia visual con el resto del repo. */}
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              {[
                { id: "basic",  label: "COORDENADAS",          grad: "linear-gradient(180deg, #ffc06e, #e4881a)", ink: "#3a2608" },
                { id: "medium", label: "PRODUCTO\nCARTESIANO", grad: "linear-gradient(180deg, #ffe97a, #d7b12a)", ink: "#3a2608" },
              ].map((lv) => {
            ...
            <div style={{
              ...
              {{
                basic:  "Ubica las coordenadas del punto dado.",
                medium: "Representa el producto A × B en el plano cartesiano.",
              }[level]}
            </div>
          </div>

          {/* Nombre + ENTRAR — segundo paso. El placeholder hace de copy
...
function CharacterScreen({ app, setApp, go }) {
  ...
  function choose() {
    // 2 niveles, mapeo level → catId/catLabel:
    //   basic  → "coords"   "Coordenadas"
    //   medium → "producto" "Producto cartesiano"
    const level = app.level || "basic";
    const map = {
      basic:  { catId: "coords",   catLabel: "Coordenadas" },
      medium: { catId: "producto", catLabel: "Producto cartesiano" },
    };
    const { catId, catLabel } = map[level] || map.basic;
    setApp((s) => ({
      ...s,
      character: sel,
...
Object.assign(window, { HomeScreen, CharacterScreen, MenuScreen, CosmosBg, CATEGORIES, incrementGamesCompleted, useVisitorCount, markFirstAttempt });
