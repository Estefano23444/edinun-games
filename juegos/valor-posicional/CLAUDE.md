...
**Juego: Valor posicional.** Carpeta autocontenida del repo
multi-juego `edinun-games`. Tres niveles: hasta 40, hasta 60, y números
de 8 o 9 cifras. La mecánica visual cambia por nivel — bloques de
Dienes para los dos primeros, slots posicionales con cartel narrado
para el avanzado. Detalle del diseño en
`.planning/valor-posicional-design.md`. Notación pedagógica:
`CMi/DMi/UMi · CM/DM/UM · C/D/U`, separadores de millar con espacio.
...
Dos modos visuales según `problem.mode` (derivado de la categoría):

**Modo Dienes (`vp40` / `vp60`):**
- State: `tens`, `units`, `pieceHistory` (para QUITAR ÚLTIMO).
- Bandeja inferior con dos botones `+ DECENA` / `+ UNIDAD`. Cap de
  decenas: 4 (vp40) o 6 (vp60). Cap de unidades: 9.
- Verificación: `tens*10 + units === problem.answer`.

**Modo Dígitos (`vp9`):**
- State: `answer` (array de 9 slots, índice 0 = CMi, índice 8 = U).
- Si `problem.digitsCount === 8`, el slot CMi queda deshabilitado.
- Numpad 0-9 abajo. Drag-and-drop a slots; tap del numpad rellena
  el primer slot vacío de izquierda a derecha (lectura natural).
  Tap en slot lleno lo borra; drag entre slots los intercambia.
- Verificación: `parseInt(answer.slice(firstActive).join(""), 10) === problem.answer`.

**Compartido entre modos:** HUD superior (logo + tabs de nivel +
cronómetro + estrellas), bocadillo de pista, personaje en esquina,
racha, modales de salir y cambio de nivel, feedback overlay,
fórmula de estrellas decreciente con tiempo. Tras 3 aciertos guarda
`lastResult` en `app` y navega a Results. El reporte usa `formatOp`
y `fmtAnswer` para imprimir "Armar 12 345 678" en vez de "12345678 VP 8".
