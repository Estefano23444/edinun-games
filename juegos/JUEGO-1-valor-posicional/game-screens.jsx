...
function DienesBar({ color = "#f5a623", small = false, ghost = false }) {
  const w = small ? 12 : 18;
  const cellH = small ? 5 : 10;
  return (
    <div style={{
      width: w, height: cellH * 10, borderRadius: 4, boxSizing: "border-box",
      background: ghost ? "transparent" : color,
      border: ghost ? `2px dashed ${color}` : "none",
      opacity: ghost ? 0.7 : 1,
      boxShadow: ghost ? "none" : `inset 0 0 0 1.5px rgba(0,0,0,0.35), 0 2px 4px rgba(0,0,0,0.4)`,
      backgroundImage: ghost ? "none" : `repeating-linear-gradient(to bottom, transparent 0, transparent ${cellH - 1}px, rgba(0,0,0,0.45) ${cellH - 1}px, rgba(0,0,0,0.45) ${cellH}px)`,
    }} />
  );
}
function DienesCube({ color = "#4fa0ff", ghost = false }) {
  return (
    <div style={{
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
      ...
      {isDienes && (() => {
        ...
        // Descomposición canónica de la respuesta (única válida con los caps:
        // unidades ≤ 9, así que las decenas son la parte alta). En el revelado
        // se usa para pintar las piezas correctas dentro de cada columna.
        const correctTens = Math.floor(problem.answer / 10);
        const correctUnits = problem.answer % 10;
        const tensOk = tens === correctTens;
        const unitsOk = units === correctUnits;
        const GREEN = "#2ecc8f", RED = "#ff6b6b";
        const fbCol = feedback === "ok" ? GREEN : feedback === "err" ? RED : null;
        const TENS_BORDER = wrongDrop === "tens" ? RED : reveal ? (tensOk ? GREEN : RED) : (fbCol || baseTens);
        const UNITS_BORDER = wrongDrop === "units" ? RED : reveal ? (unitsOk ? GREEN : RED) : (fbCol || baseUnits);
        ...
        return (
          <React.Fragment>
            ...
            <div style={{
              ...
              {/* El revelado de la respuesta se muestra DENTRO de las columnas
                  (piezas verdes = lo correcto, fantasma = lo que faltaba, rojo
                  = lo que sobra), no como un cartel flotante. */}
            </div>
            ...
            <div style={{
              ...
              >
                ...
                <div style={{
                  ...
                  {reveal ? (
                    <React.Fragment>
                      <span style={{ color: tensOk ? GREEN : RED }}>{tens}</span>
                      <span style={{ color: "rgba(246,241,255,0.6)", fontSize: 13 }}> / {correctTens}</span>
                    </React.Fragment>
                  ) : (
                    <React.Fragment>{tens} <span style={{ color: "rgba(246,241,255,0.55)", fontSize: 13 }}>· {tens * 10}</span></React.Fragment>
                  )}
                </div>
                <div style={{
                  ...
                  {reveal
                    ? Array.from({ length: Math.max(tens, correctTens) }).map((_, i) => (
                        <DienesBar key={i}
                          color={i < correctTens ? GREEN : RED}
                          ghost={i >= tens && i < correctTens} />
                      ))
              ...
              >
                ...
                <div style={{
                  ...
                  {reveal ? (
                    <React.Fragment>
                      <span style={{ color: unitsOk ? GREEN : RED }}>{units}</span>
                      <span style={{ color: "rgba(246,241,255,0.6)", fontSize: 13 }}> / {correctUnits}</span>
                    </React.Fragment>
                  ) : (
                    <React.Fragment>{units} <span style={{ color: "rgba(246,241,255,0.55)", fontSize: 13 }}>· {units}</span></React.Fragment>
                  )}
                </div>
                <div style={{
                  ...
                  {reveal
                    ? Array.from({ length: Math.max(units, correctUnits) }).map((_, i) => (
                        <DienesCube key={i}
                          color={i < correctUnits ? GREEN : RED}
                          ghost={i >= units && i < correctUnits} />
                      ))
      ...
      {!isDienes && (() => {
        ...
        const SLOT_W = 46, SLOT_H = 140, GAP = 3;
        const GROUP_GAP = 10;
        return (
          <React.Fragment>
            ...
            <div style={{
              ...
              {/* El revelado de la respuesta se muestra DENTRO de los slots:
                  cada casillero pinta su dígito correcto en verde, con borde
                  verde/rojo según si el niño acertó ese casillero. */}
            </div>
            ...
            <div style={{
              position: "absolute", top: 240, left: 232, right: 200,
              textAlign: "center",
            }}>
              ...
              <div style={{
                ...
                {SLOT_LABELS.map((_lb, i) => {
                  ...
                  // Dígito correcto de este casillero (vacío si está deshabilitado).
                  const correctDigit = disabled ? "" : String(problem.answer)[i - firstActive];
                  const slotOk = !disabled && String(d) === correctDigit;
                  return (
                    <React.Fragment key={`S${i}`}>
                      ...
                      <div
                        ...
                        style={{
                          width: SLOT_W, height: SLOT_H, fontSize: 50,
                          marginRight: i < SLOT_LABELS.length - 1 ? GAP : 0,
                          opacity: disabled ? 0.25 : 1,
                          pointerEvents: disabled ? "none" : "auto",
                          // Durante el revelado cada casillero muestra el dígito
                          // CORRECTO en verde y, si el niño falló, debajo en rojo
                          // y tachado lo que él había puesto. Borde verde/rojo.
                          borderColor: reveal ? (slotOk ? "#2ecc8f" : "#ff6b6b") : feedback === "ok" ? "#2ecc8f" : feedback === "err" ? "#ff6b6b" : undefined,
                          boxShadow: reveal
                            ? (slotOk ? "0 0 18px rgba(46,204,143,0.65)" : "0 0 18px rgba(255,107,107,0.65)")
                            : feedback === "ok"
                            ? "0 0 18px rgba(46,204,143,0.65)"
                            : feedback === "err"
                              ? "0 0 18px rgba(255,107,107,0.65)"
                              : undefined,
                          color: reveal && !disabled ? "#7CF5C0" : "#fff",
                          cursor: disabled ? "not-allowed" : (filled ? "grab" : "default"),
                        }}
                      >
                        {disabled ? "—" : reveal ? (
                          slotOk ? (
                            <span style={{ color: "#7CF5C0" }}>{correctDigit}</span>
                          ) : (
                            <span style={{ display: "inline-flex", flexDirection: "column", alignItems: "center", lineHeight: 1 }}>
                              <span style={{ color: "#7CF5C0", fontWeight: 800 }}>{correctDigit}</span>
                              <span style={{ fontSize: 16, color: "#ff8a8a", textDecoration: "line-through", marginTop: 3 }}>{filled ? d : "·"}</span>
                            </span>
                          )
                        ) : (filled ? d : "")}
                      </div>
