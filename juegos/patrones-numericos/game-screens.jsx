...
//   idx 1 → multiplicación = tabla del N · factor 2..10
//   idx 2 → división = tabla del N inversa · factor 2..10
// Restricción: solo números naturales (≥ 1) en TODOS los términos.
//
// `easy` modula el rango. Si easy=true → rangos pequeños (paso 2..5,
// factor 2..4); si easy=false → rangos grandes (paso 6..11, factor 5..10).
// El llamador (GameScreen) decide aleatoriamente al inicio de la sesión
// cuál de las 3 rondas será la fácil — las otras dos serán difíciles.
// ─────────────────────────────────────────────────────────────
function makeProblem(cat, idx = 0, easy = false) {
  ...
  if (idx === 1) {
    // Multiplicación = tabla del N · 5 múltiplos consecutivos: N, 2N, 3N, 4N, 5N.
    // Easy → factor 2..4; Hard → factor 5..10.
    // Pedagógicamente alineado con "tablas" del 2do ciclo escolar.
    const factor = easy ? rand(2, 4) : rand(5, 10);
    const terms = Array.from({ length: len }, (_, i) => factor * (i + 1));
    return { type: "patrones", idx, op: "×", step: factor, terms, blanks: pickBlanks(len, blankCount) };
  }

  // idx === 2 — División = tabla del N inversa: 5N, 4N, 3N, 2N, N.
  const divisor = easy ? rand(2, 4) : rand(5, 10);
  const terms = Array.from({ length: len }, (_, i) => divisor * (len - i));
...
function GameScreen({ app, setApp, go }) {
  ...
  function verify() {
    ...
    setTimeout(() => {
      ...
      if (newAttempted >= 3) {
        ...
        // Esto controla si la siguiente ronda es +/−, ×, o ÷.
        // Y si es la ronda fácil sorteada al inicio, se generan rangos pequeños.
        setProblem(makeProblem(cat, newAttempted, newAttempted === easyRound.current));
  ...
  // Tile width según largo del término (para que las casillas de la secuencia
  // mantengan proporciones legibles).
  function termTileWidth(t) {
    const len = String(t).length;
    return Math.max(50, len * 28 + 20);
  }
          ...
          {problem.terms.map((t, i) => {
            ...
            const cell = blankIdx === -1 ? (
              <div style={{
                ...
                padding: "10px 12px",
                fontFamily: "var(--ed-font-display)", fontWeight: 700,
                fontSize: 40, lineHeight: 1, color: "#fce9a8",
              ...
              return (
                <div style={{ display: "flex", gap: 4 }}>
                  {Array.from({ length: slotsLen }).map((_, j) => (
                    <SlotBox key={j} value={slots[j] || ""} feedback={feedback} size={44} />
                  ))}
                </div>
              );
            })();
