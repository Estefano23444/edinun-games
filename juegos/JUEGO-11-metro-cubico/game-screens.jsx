// game-screens.jsx — Juego de Metro cúbico (1 nivel, 3 rondas).
// Audiencia 10 años (excepción al default 6-8 del repo).
// Múltiplos y submúltiplos del m³ con factor ×1000 entre adyacentes.
...
// ─────────────────────────────────────────────────────────────
// Generador de problemas — 1 categoría ("volumen"), 3 rondas.
//   idx 0 → SOLO multiplicación (×). Par aleatorio en la escala
//           km³ → hm³ → dam³ → m³ → dm³ → cm³ → mm³, dirección
//           mayor → menor. Factor ×1000 (adyacentes), ×1.000.000
//           (2 saltos), ×1.000.000.000 (3 saltos).
//   idx 1 → SOLO división (÷). Mismo sorteo de pares, dirección
//           menor → mayor.
//   idx 2 → 1 dm³ = 1 L  ·  1 mL = 1 cm³ (las 2 equivalencias que
//           el chico conoce a 10 años, conversión 1:1).
//
// Restricciones:
// - Tope 10 cifras (≤ 9.999.999.999) en TODOS los valores. Necesario
//   para que km³↔m³ (factor 1.000.000.000) sea posible.
// - Solo respuestas enteras. En ronda 2 el fromValue se construye
//   como answer × factor para garantizarlo.
// - Saltos máximos: 3 (factor 10^9). Saltos mayores no caben en el tope.
// - Para 1-2 saltos: fromValue 1-99. Para 3 saltos: fromValue 1-9.
// ─────────────────────────────────────────────────────────────
const VOLUME_SCALE = ["km³", "hm³", "dam³", "m³", "dm³", "cm³", "mm³"];

function makeProblem(cat, idx = 0) {
  const rand = (a, b) => Math.floor(Math.random() * (b - a + 1)) + a;

  if (idx === 0) {
    // Ronda 1 — multiplicación. Sortea par mayor → menor con saltos 1..3.
    let big, small, jumps;
    do {
      big = rand(0, VOLUME_SCALE.length - 2);
      small = rand(big + 1, VOLUME_SCALE.length - 1);
      jumps = small - big;
    } while (jumps > 3);

    const factor = Math.pow(1000, jumps);
    // Cap fromValue según saltos para mantener answer ≤ 10 cifras.
    const fromValue = jumps <= 2 ? rand(1, 99) : rand(1, 9);
    const answer = fromValue * factor;
    return {
      type: "volumen", idx,
      fromValue, fromUnit: VOLUME_SCALE[big],
      toUnit: VOLUME_SCALE[small],
      answer,
      showFactor: false,
    };
  }
  ...
  if (idx === 1) {
    // Ronda 2 — división. Mismo sorteo de pares, dirección invertida.
    ...
    // según permita el factor + tope 10 cifras).
    let big, small, jumps;
    do {
      big = rand(0, VOLUME_SCALE.length - 2);
      small = rand(big + 1, VOLUME_SCALE.length - 1);
    ...
    const factor = Math.pow(1000, jumps);
    const maxAnswer = Math.floor(9999999999 / factor);
    ...
    return {
      type: "volumen", idx,
      fromValue, fromUnit: VOLUME_SCALE[small],
      toUnit: VOLUME_SCALE[big],
      answer,
      showFactor: false,
    };
  }

  // idx === 2 — Equivalencias con litros (1:1).
  // Las únicas que un chico de 10 años conoce:
  //   1 dm³ = 1 L     1 mL = 1 cm³
  // Sortear cuál equivalencia y dirección. Valor 1-999 para variar dígitos.
  const equiv = rand(0, 1); // 0 = dm³ ↔ L,  1 = cm³ ↔ mL
  const dirA = Math.random() < 0.5;
  const value = rand(1, 999);

  if (equiv === 0) {
    const fromUnit = dirA ? "dm³" : "L";
    const toUnit = dirA ? "L" : "dm³";
    return {
      type: "volumen", idx,
      fromValue: value, fromUnit,
      toUnit,
      answer: value,
      showFactor: true,
    };
  }
  const fromUnit = dirA ? "cm³" : "mL";
  const toUnit = dirA ? "mL" : "cm³";
  return {
    type: "volumen", idx,
    fromValue: value, fromUnit,
    toUnit,
...
const ENCOURAGEMENTS = [
  ...
  "Recuerda: cada salto vale ×1.000.",
  "¡La próxima es tuya!",
...
// ─────────────────────────────────────────────────────────────
// Tabla de unidades de volumen — referencia visual para rondas
// métricas (1 y 2). Estilo "tabla del libro": celdas en fila, de
// mayor (km³) a menor (mm³). Factor ×1000 entre adyacentes.
// En ronda 3 se reemplaza por la píldora con las 2 equivalencias.
// ─────────────────────────────────────────────────────────────
function UnitsTable() {
...
function GameScreen({ app, setApp, go }) {
  ...
  const cat = app.currentCategory || "volumen";
  const catLabel = app.currentCatLabel || "Múltiplos y submúltiplos de metro cúbico";
  ...
  // Fase "reveal": antes del overlay "¡UPS!" se revela la respuesta correcta.
  // Respuesta escrita (numpad multi-dígito): los slots conservan lo que escribió
  // el niño (en rojo) y aparece un cartel verde con el número correcto completo.
  ...
  function verify() {
    ...
    if (!isCorrect) {
      // Conservar lo que escribió el niño (los slots quedan en rojo) y mostrar
      // el número correcto completo en un cartel verde aparte, antes del "¡UPS!".
  ...
  const bocadilloText = "Convierte el volumen a la unidad pedida.";
  const instructionText = problem.idx === 2 ? "📦 Cubos y litros" : "📦 ¡Llena el cubo!";

  // Tile width / fontSize según largo del valor visible.
  // En R2 fromValue puede llegar a 10 cifras (factor 10^9 + answer 9).
  ...
  function slotSize(slotsLen) {
    ...
    if (slotsLen === 6) return 30;
    if (slotsLen === 7) return 28;
    if (slotsLen === 8) return 26;
    if (slotsLen === 9) return 24;
    return 22; // 10 slots
  }
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
      ...
      <div data-qa="zona-central" style={{
        ...
        {/* Contenido interactivo de la ronda, centrado verticalmente. */}
        ...
        {/* Guía visual:
            - Rondas 1, 2 (idx 0, 1): tabla de unidades de volumen.
            - Ronda 3 (idx 2): píldora con las 2 equivalencias de litros. */}
        {problem.showFactor ? (
          <div style={{
        ...
        {/* Ecuación: "5 m³ = [_ _ _ _] dm³" — fila + cartel de respuesta
            correcta apilados en columna, para que el cartel caiga DEBAJO y no
            invada la columna de botones (la respuesta puede tener 10 cifras). */}
        <div style={{
        ...
        <div style={{
          ...
          {/* Slots — al fallar conservan lo escrito por el niño, marcados en rojo. */}
          ...
          {/* Al fallar: cartel verde con el número correcto completo, DEBAJO
...
function ResultsScreen({ app, setApp, go }) {
  const char = CHARACTERS.find((c) => c.id === app.character) || CHARACTERS[0];
  const res = app.lastResult || { category: "Múltiplos y submúltiplos de metro cúbico", solved: 0, total: 3, time: 0, starsEarned: 0, log: [] };
