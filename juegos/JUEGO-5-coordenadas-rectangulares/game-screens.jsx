...
function makeProblem(cat, idx = 0) {
  ...
  if (idx === 1) {
    // Plano numérico 0..5 × 0..5. Un punto marcado.
    const x = rand(0, 5);
    const y = rand(0, 5);
    const correct = `(${x}, ${y})`;
    ...
    while (distractors.size < 3 && guard < 30) {
      ...
      let dx = x + rand(-2, 2);
      let dy = y + rand(-2, 2);
      dx = Math.max(0, Math.min(5, dx));
      dy = Math.max(0, Math.min(5, dy));
      const opt = `(${dx}, ${dy})`;
      if (opt !== correct) distractors.add(opt);
    }
  ...
  // R3 — camino del perro Kody. Personaje en (0,0), tesoro en (X, Y).
  const X = rand(2, 6);
  const Y = rand(2, 5);
  return {
    type: "coords", idx,
...
function CartesianPlane({ x, y, max = 5, point = true, character = false, walkedX = 0, walkedY = 0 }) {
  const step = 36;
  const padL = 30, padB = 26, padT = 12, padR = 12;
    ...
    <svg width={w} height={h} style={{
      ...
      {Array.from({ length: max + 1 }, (_, i) => (
        <g key={`v-${i}`}>
          ...
          <text x={toX(i)} y={toY(0) + 16} fontSize="11" fill="#fce9a8" textAnchor="middle"
      ...
      {Array.from({ length: max + 1 }, (_, i) => (
        <g key={`h-${i}`}>
          <line x1={toX(0)} y1={toY(i)} x2={toX(max)} y2={toY(i)}
            stroke="rgba(252,233,168,0.18)" strokeWidth="1" />
          <text x={toX(0) - 8} y={toY(i) + 4} fontSize="11" fill="#fce9a8" textAnchor="end"
            fontFamily="var(--ed-font-display)" fontWeight="700">{i}</text>
        </g>
      ))}
      {/* Ejes */}
      ...
      <text x={toX(max) + 4} y={toY(0) + 4} fontSize="11" fill="#fce9a8"
        fontFamily="var(--ed-font-display)" fontWeight="700">x</text>
      <text x={toX(0) - 4} y={toY(max) - 2} fontSize="11" fill="#fce9a8" textAnchor="end"
        fontFamily="var(--ed-font-display)" fontWeight="700">y</text>
      ...
      {/* Personaje en origen (R3) */}
      {character && (
        <text x={toX(0)} y={toY(0) - 14} fontSize="22" textAnchor="middle">🐶</text>
      )}
      {/* Tesoro en target (R3) */}
      {character && (
        <text x={toX(x)} y={toY(y) + 6} fontSize="20" textAnchor="middle">💎</text>
      )}
...
function GameScreen({ app, setApp, go }) {
  ...
  function pressDigit(d) {
    if (problem.idx !== 2) return;
    if (slot === "" && d === "0") return; // sin leading zero
    if (slot.length < 1) setSlot(slot + d);
  }
  function eraseLast() {
    if (problem.idx !== 2) return;
  ...
  function restartGame() {
    ...
    setLog([]); setSlot(""); setStep(0); setWalkedX(0);
  ...
  const instructionByIdx = [
    ...
    step === 0 ? "¿Cuántos pasos a la derecha?" : "¿Cuántos pasos hacia arriba?",
  ];
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
      ...
      <div data-qa="zona-central" style={{
        ...
        width: 580, height: problem.idx === 2 ? 320 : 360,
        ...
        {problem.idx === 1 && (
          <>
            <CartesianPlane x={problem.x} y={problem.y} max={5} point={true} />
            <div style={{
              display: "grid", gridTemplateColumns: "repeat(2, 1fr)",
              gap: 10, width: 360,
            }}>
