...
function GameScreen({ app, setApp, go }) {
  ...
  // Como esta mecánica es numpad/respuesta escrita, el slot CONSERVA lo que
  // escribió el niño (en rojo) y aparece un cartel verde aparte con el valor
  // correcto de cada hueco: { correctVals: [n, ...] }.
  ...
  function verify() {
    ...
    for (let bi = 0; bi < blankInputs.length; bi++) {
      ...
      correctVals.push(correctVal);
      if (userVal !== correctVal) isCorrect = false;
    }

    if (typeof window.markFirstAttempt === "function") window.markFirstAttempt();

    // Al fallar: revelar la respuesta correcta ANTES del "¡UPS!".
    // El slot conserva lo que escribió el niño (en rojo) y aparece un cartel
    // verde con los valores correctos. Bloquea inputs durante el revelado.
    if (!isCorrect) {
      setReveal({ correctVals });
      setTimeout(() => { setReveal(null); finalize(userVals, false); }, REVEAL_MS);
      return;
    }
    finalize(userVals, true);
  }

  function finalize(userVals, isCorrect) {
  ...
  const bocadilloText = "¿Qué número sigue la secuencia?";
  const instructionText = "🔍 ¡Descubre el patrón!";
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
          ...
          <div className="ed-float-soft" style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            ...
            <div style={{ position: "relative", width: 215, marginBottom: 8, zIndex: 2 }}>
              <div style={{
                ...
                {bocadilloText}
                <div style={{ marginTop: 6, fontSize: 12, fontWeight: 700, color: "rgba(191,245,223,0.92)" }}>
                  👆 Toca los números de abajo.
                </div>
                <div style={{
      ...
      {/* ══════ ZONA CENTRAL — enunciado fijo arriba (debajo del HUD) y la
          secuencia interactiva centrada en el espacio restante. Antes el
      ...
      <div data-qa="zona-central" style={{
        ...
        <div style={{
          ...
          {problem.terms.map((t, i) => {
            ...
            const cell = blankIdx === -1 ? (
              ...
              // Durante el revelado, el slot conserva lo que escribió el niño
              // pero se marca en rojo (la respuesta correcta va aparte, abajo).
              const slotFeedback = reveal && reveal.correctVals ? "err" : feedback;
              return (
                <div style={{ display: "flex", gap: 4 }}>
                  {Array.from({ length: slotsLen }).map((_, j) => (
                    <SlotBox key={j} value={slots[j] || ""} feedback={slotFeedback} size={ss} />
                  ))}
                </div>
              );
            })();
        ...
        {/* Revelado: cartel verde con la(s) respuesta(s) correcta(s) antes del
            "¡UPS!". El slot conserva lo que escribió el niño (en rojo) y aquí
            se muestra el valor correcto, sin sobrescribir su intento. */}
        {reveal && reveal.correctVals && (
          <div style={{
            ...
            <span style={{ fontSize: 13, color: "#bff5df", letterSpacing: "0.03em", textTransform: "uppercase" }}>
              {reveal.correctVals.length > 1 ? "Correctas:" : "Correcta:"}
            </span>
            ✓ {reveal.correctVals.join(", ")}
          </div>
        )}
        </div>
      </div>
