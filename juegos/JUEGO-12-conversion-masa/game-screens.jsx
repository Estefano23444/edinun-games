...
function makeProblem(cat, idx = 0) {
  ...
  if (idx === 1) {
    // R2 — ¿Cuál NO equivale? Pool curado donde TODA fuente tiene
    // EXACTAMENTE 3 equivalencias simples (sin decimales raros, sin mg).
    // 3 correctas + 1 falsa = 4 opciones en grid 2x2.
    const sources = [
      ...
      { unit: "kg", value: 500, equivalences: [
        { unit: "t", value: 0.5 },
        { unit: "g", value: 500000 },
        { unit: "lb", value: 1100 },
      ]},
      ...
      { unit: "kg", value: 2000, equivalences: [
        { unit: "t", value: 2 },
        { unit: "g", value: 2000000 },
        { unit: "lb", value: 4400 },
      ]},
      { unit: "t", value: 0.1, equivalences: [
        { unit: "kg", value: 100 },
        { unit: "g", value: 100000 },
        { unit: "lb", value: 220 },
      ]},
      { unit: "t", value: 0.5, equivalences: [
        { unit: "kg", value: 500 },
        { unit: "g", value: 500000 },
        { unit: "lb", value: 1100 },
      ]},
      ...
      { unit: "t", value: 5, equivalences: [
        { unit: "kg", value: 5000 },
        { unit: "g", value: 5000000 },
        { unit: "lb", value: 11000 },
      ]},
      { unit: "g", value: 100000, equivalences: [
        { unit: "kg", value: 100 },
        { unit: "t", value: 0.1 },
        { unit: "lb", value: 220 },
      ]},
      { unit: "g", value: 500000, equivalences: [
        { unit: "kg", value: 500 },
        { unit: "t", value: 0.5 },
        { unit: "lb", value: 1100 },
      ]},
      { unit: "g", value: 1000000, equivalences: [
        { unit: "kg", value: 1000 },
        { unit: "t", value: 1 },
        { unit: "lb", value: 2200 },
      ]},
      { unit: "lb", value: 220, equivalences: [
        { unit: "kg", value: 100 },
        { unit: "t", value: 0.1 },
        { unit: "g", value: 100000 },
      ]},
      { unit: "lb", value: 1100, equivalences: [
        { unit: "kg", value: 500 },
        { unit: "t", value: 0.5 },
        { unit: "g", value: 500000 },
      ]},
      { unit: "lb", value: 2200, equivalences: [
        { unit: "kg", value: 1000 },
        { unit: "t", value: 1 },
        { unit: "g", value: 1000000 },
      ]},
    ...
    // Las 3 correctas son TODAS las equivalencias del source.
    const corrects = src.equivalences.slice();

    // Falsa: tomar una de las correctas y moverle los ceros (×10, ×100, ÷10).
...
function GameScreen({ app, setApp, go }) {
  ...
  // R1/R2 (opción múltiple): { correctKey?, correctIdx?, wrongKey?, wrongIdx? }
  //   → la opción correcta en verde con ✓, la elegida en rojo.
  // R3 (numpad): { correctSlot: "<valor correcto>" } → los slots conservan lo
  //   que escribió el niño (en rojo) y aparece un cartel verde con el número correcto.
  ...
  function answerTapR1(chosen) {
    if (problem.idx !== 0 || feedback !== null || reveal !== null) return;
    ...
    if (!isCorrect) {
      // Marcar la opción correcta (verde ✓) y la elegida (roja) antes del "¡UPS!".
      // En R1 el lado correcto baja la balanza, así que mantenemos el tilt visible.
      setReveal({ correctKey: problem.outcome, wrongKey: chosen });
  ...
  function answerTapR2(chosenIdx) {
    if (problem.idx !== 1 || feedback !== null || reveal !== null) return;
    ...
    if (!isCorrect) {
      // Marcar la opción correcta (la falsa, en verde ✓) y la elegida (roja) antes del "¡UPS!".
      const correctIdx = problem.options.findIndex((o) => o.isFalse);
      setReveal({ correctIdx, wrongIdx: chosenIdx });
      setTimeout(() => { setReveal(null); finalize(false, entry); }, REVEAL_MS);
      return;
    }
    finalize(isCorrect, entry);
  ...
  function verify() {
    ...
    if (Math.abs(userVal - target) > 0.0001) {
      ...
      // Conservar lo que escribió el niño (los slots, en rojo) y mostrar el
      // número correcto del paso actual en un cartel verde aparte, antes del "¡UPS!".
      const correctUnit = currentStepIdx === 0 ? problem.midUnit : problem.finalUnit;
      setReveal({ correctSlot: `${fmt(target)} ${correctUnit}` });
      setTimeout(() => {
        setReveal(null);
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
      ...
      {/* ══════ ZONA CENTRAL ══════
          Enunciado fijo arriba (debajo del HUD) y el contenido interactivo
          centrado en el espacio restante. Antes el enunciado iba dentro de un
          space-evenly y quedaba flotando con un hueco arriba (donde antes vivía
          el indicador de RONDA, ahora movido al logo). */}
      <div data-qa="zona-central" style={{
        ...
        {/* Contenido interactivo de cada ronda, centrado verticalmente.
            En R1 (balanza) hay solo 2 bloques: con space-evenly quedaban
            pegados a los extremos dejando un hueco enorme en medio. Para esa
            ronda los agrupamos centrados con un gap fijo; el resto mantiene
            space-evenly porque tienen más contenido. */}
        <div data-qa="zona-contenido" style={{
          flex: 1, width: "100%",
          display: "flex", flexDirection: "column",
          alignItems: "center",
          justifyContent: problem.idx === 0 ? "center" : "space-evenly",
          gap: problem.idx === 0 ? 34 : 0,
        }}>
        ...
        {problem.idx === 0 && (() => {
          ...
          // Se inclina tanto al resolver (feedback) como al revelar la correcta.
          const tilt = (feedback !== null || reveal !== null)
            ? (problem.outcome === "left" ? -6 : problem.outcome === "right" ? 6 : 0)
            : 0;
          ...
          return (
            <>
              {/* Contenedor de balanza con altura fija para evitar overlap
                  (recortado: el gráfico solo ocupa ~135px, antes sobraban ~65). */}
              <div style={{
                position: "relative",
                ...
                <div style={{
                  ...
                  <div
                    ...
                    style={{
                      ...
                      filter: locked ? "none" : "drop-shadow(0 0 0 rgba(79,216,255,0))",
                    }}
                    onMouseEnter={(e) => {
                      if (!locked) {
                ...
                <div style={{
                  ...
                  transform: "translateX(-50%)",
                  width: 86, height: 8, borderRadius: 4,
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, width: 540 }}>
                {[
                  ...
                  const isCorrectOpt = reveal && reveal.correctKey != null && btn.key === reveal.correctKey;
                  const isWrongOpt = reveal && reveal.wrongKey != null && btn.key === reveal.wrongKey;
                  return (
                  <button
                    key={btn.key}
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 12, width: 480 }}>
              {problem.options.map((op, i) => {
                const isCorrectOpt = reveal && reveal.correctIdx != null && i === reveal.correctIdx;
                const isWrongOpt = reveal && reveal.wrongIdx != null && i === reveal.wrongIdx;
                const locked = feedback !== null || reveal !== null;
                return (
                <button
                  key={i}
