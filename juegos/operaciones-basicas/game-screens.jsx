...
function GameScreen({ app, setApp, go }) {
  ...
  // El niño conserva lo que escribió (slots en rojo) y aparece un cartel verde
  // "Correcta: ✓ <resultado>" aparte (no sobrescribimos su input). Mismo patrón
  // que JUEGO-5. Cuando reveal !== null, press/erase quedan bloqueados.
  const [reveal, setReveal] = useStateG(null); // null | { correctAnswer }
  // El revelado es el momento educativo → dura más. El overlay "¡UPS!"
  // posterior es solo refuerzo emocional → corto (ver `wait` en finalize).
  ...
  function verify() {
    if (reveal) return; // bloqueado durante el revelado de la respuesta
    ...
    if (!isCorrect) {
      // Revelar el RESULTADO correcto (cartel verde aparte) conservando lo que
      // escribió el niño (slots en rojo). Tras REVEAL_MS, finalizar incorrecto.
      setReveal({ correctAnswer: problem.answer });
  ...
  function finalize(isCorrect, partialEntry) {
    ...
    // El revelado ya mostró la correcta (REVEAL_MS); el "¡UPS!" es corto.
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
      ...
      {(() => {
        ...
        return (
          <div style={{
            // El enunciado queda fijo arriba (justo bajo el HUD) y el grid de la
            // ecuación se centra en el espacio restante. Antes el wrapper iba a
            // top:140 sin centrado vertical, dejando un hueco arriba (donde antes
            // vivía el indicador de RONDA, ahora pegado al logo). Patrón JUEGO-5:
            // enunciado fijo + contenido envuelto en un flex:1 space-evenly.
            ...
            {/* Contenido interactivo (ecuación CDU) centrado verticalmente en el
                espacio restante bajo el enunciado. */}
            <div style={{
              flex: 1, width: "100%",
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      margin: "0 auto",
                      // Durante el revelado conservamos lo que escribió el niño
                      // marcándolo en rojo (la correcta va en un cartel verde aparte).
            ...
            {/* Revelado: cartel verde con el RESULTADO correcto, aparte de los
                slots del niño (que quedan en rojo). Patrón JUEGO-5. */}
            {reveal && (
              <div style={{
                marginTop: 14,
                ...
                boxShadow: "0 0 18px rgba(46,204,143,0.5)",
                whiteSpace: "nowrap",
              }}>
                <span style={{ fontSize: 15, color: "#bff5df", letterSpacing: "0.03em", fontWeight: 700 }}>Correcta:</span>
